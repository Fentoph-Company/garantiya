...
export default function WarrantiesPage() {
  ...
  async function del(id:string){if(!confirm("Ushbu garantiya talonini o‘chirishni tasdiqlaysizmi? Bu amal uni xaridor uchun yaroqsiz qiladi."))return;const r=await fetch("/api/admin/warranties?id="+id,{method:"DELETE"});if(!r.ok){const d=await r.json();setError(d.error||"O‘chirilmadi.");return}load();}
  ...
  return <section className="panel-content">
    ...
    <div className="panel-card"><div className="panel-list">
      {items.length===0?<div className="empty-state">Hozircha kafolat talonlari mavjud emas.</div>:items.map(w=><div className="warranty-row" key={w.id}>
        ...
        <div className="warranty-meta"><span>{new Date(w.warrantyTo).toLocaleDateString("uz-UZ")}</span><button className="secondary-button" onClick={()=>del(w.id)}>O‘chirish</button>}</div>
      </div>)}
    </div></div>
  </section>;
}
